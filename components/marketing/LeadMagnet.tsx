...
export function LeadMagnet() {
  ...
  const handleDownload = () => {
    if (email) {
      toast.success("Guia enviada a tu email. Revisa tu bandeja.");
      setEmail("");
      setIsOpen(false);
    } else {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        ...
        >
          ...
          Guia Gratuita: 7 Hacks IA para Triplicar Conversiones
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-md bg-brand-900/95 border-brand-700/50 backdrop-blur-sm text-white">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold text-white">Descarga tu Guia Gratuita</DialogTitle>
